...
function loopAndModifyArray(array, condition, modifier, depth, processed) {
    ...
    if (processed.has(array)) {
        console.log('!! Reference to already processed array, not entering.');
        return array;
    }
    processed.add(array);
    array.forEach((el, i) => {
        if (el === array) return console.log('!!! Circular array detected, not entering.');
...
function loopAndModifyObject(object, condition, modifer, depth, processed) {
    ...
    if (processed.has(object)) {
        console.log('!! Reference to already processed object, not entering.');
        return object;
    }
    ...
    for (const key in object) {
        if (object.hasOwnProperty(key)) {
            ...
            if (val === object) {
                circular = true;
                console.log('!!! Circular object detected, not entering.');
            }
            if (!circular) {
